...
func (f *Factory) NewOtp(mods ...OtpMod) *OtpTemplate {
	return f.NewOtpWithContext(context.Background(), mods...)
}

func (f *Factory) NewOtpWithContext(ctx context.Context, mods ...OtpMod) *OtpTemplate {
	o := &OtpTemplate{f: f}

	if f != nil {
		f.baseOtpMods.Apply(ctx, o)
	}

	OtpModSlice(mods).Apply(ctx, o)

	return o
}

func (f *Factory) FromExistingOtp(m *models.Otp) *OtpTemplate {
	o := &OtpTemplate{f: f, alreadyPersisted: true}

	o.ID = func() int64 { return m.ID }
	o.UserID = func() int64 { return m.UserID }
	o.Code = func() string { return m.Code }
	o.Purpose = func() string { return m.Purpose }
	...
	if m.R.User != nil {
		OtpMods.WithExistingUser(m.R.User).Apply(ctx, o)
	}
...
func (f *Factory) FromExistingUser(m *models.User) *UserTemplate {
	...
	ctx := context.Background()
	if len(m.R.Otps) > 0 {
		UserMods.AddExistingOtps(m.R.Otps...).Apply(ctx, o)
	}
...
func (f *Factory) ClearBaseOtpMods() {
	f.baseOtpMods = nil
}

func (f *Factory) AddBaseOtpMod(mods ...OtpMod) {
	f.baseOtpMods = append(f.baseOtpMods, mods...)
}
